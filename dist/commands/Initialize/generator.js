...
const generateAppFolder = (option) => {
  ...
  if (option["useWebpack"]) {
    ...
    ] = `webpack --config ${option["appName"]}/webpack.config.js`;
    jsonfile_1.writeFileSync(`package.json`, packageJSON, {
      spaces: 4,
      EOL: "\r\n",
    });
    ...
    fs_1.writeFileSync(
      ...
      sampleCode_1.generateSample(option)
    );
    spawnSync(
      "npx",
      [
  ...
  if (option["type"] === "Plugin") {
    ...
    fs_1.writeFileSync(
      `${option["appName"]}/source/js/config.${extension}`,
      ""
    );
